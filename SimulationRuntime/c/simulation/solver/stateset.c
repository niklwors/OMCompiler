...
/*! \fn initialStateSelection KAB:WIP
 *
 *  function to select the states for initialization for an individual stateSet
 *
 *  \param [ref] [data]
 *  \param [in]  [reportError]
 *  \return      ???
 ...
 *  \author Abdelhak / Frenkel TUD
 */
//int initialStateSelection(DATA *data, threadData_t *threadData, char reportError, long setIndex)
//{
//    long j=0;
//    long k=0;
//    long l=0;
...
//    if(ACTIVE_STREAM(LOG_DSS))
//    {
//      infoStreamPrint(LOG_DSS, 1, "initialStateSelection Set %ld at time = %f", setIndex, data->localData[0]->timeValue);
//      printStateSelectionInfo(data, set);
//      messageClose(LOG_DSS);
//    }
//    /* generate jacobian, stored in set->J */
...
//    /* if we have a new set throw event for reinitialization
//       and set the A matrix for set.x=A*(states) */
//    res = comparePivot(oldColPivot, set->colPivot, set->nCandidates, set->nDummyStates, set->nStates, set->F, set->states, set->statescandidates, data, switchStates);
//    memcpy(set->colPivot, oldColPivot, set->nCandidates*sizeof(modelica_integer));
//    memcpy(set->rowPivot, oldRowPivot, set->nDummyStates*sizeof(modelica_integer));
//
//    free(oldColPivot);
